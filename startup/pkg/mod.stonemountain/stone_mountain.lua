...
function sm_sys:entity_init()
    for e in w:select "INIT stonemountain_aabb:in render_object?update bounding:in eid:in" do
        local idx = e.stonemountain_aabb
        aabb_table[idx] = math3d.ref(e.bounding.aabb)
        w:remove(e.eid)
    end

...
local _get_hitch_children ; do
    ...
    local function _create_animation(group_id, prefab_file_name, pose, animation_name, process)
        local animation_inst = ecs.create_instance(prefab_file_name:string())
...
local M = {}
function M.create(prefab_file_name, cull_group_id, state, color, srt, slot, pose, parent)
    ...
    local function attach(self, slot_name, model)
        ...
        self.slot_attach[slot_name] = M.create(model, cull_group_id, "opaque", COLOR_INVALID, {}, s.slot, prefab_binding.pose, self.hitch_entity_object.id)
    end

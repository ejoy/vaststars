...
return function(start)
    ...
    if start.teardown then
        ...
        v.command = "teardown"
        v.background_image = "textures/building-menu-longpress/teardown.texture"
    ...
    if start.move then
        local v = setmetatable({}, {__index = DEFAULT})
        v.command = "move"
        v.background_image = "textures/building-menu-longpress/move.texture"
        start.buttons[#start.buttons + 1] = v
    end
    ...
    if start.copy then
        local v = setmetatable({}, {__index = DEFAULT})
        v.command = "copy"
        v.background_image = "textures/building-menu-longpress/clone.texture"
        start.buttons[#start.buttons + 1] = v
    end
